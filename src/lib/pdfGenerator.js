...
const DISCLAIMER = 'Documento generado con fines educativos. Datos ficticios. Consulte a un contador certificado para presentaciones reales ante la DIAN.';
...
function addHeader(doc, formNum, formTitle, period, company) {
  ...
  doc.text('REPÚBLICA DE COLOMBIA · DIRECCIÓN DE IMPUESTOS Y ADUANAS NACIONALES — DIAN', w / 2, 9, { align: 'center' });
...
function ivaBreakdownByRate(invoices = []) {
  const groups = {};
  invoices.forEach(inv => {
    ...
    groups[key].iva += Number(inv.iva_amount) || 0;
    groups[key].count += 1;
  });
...
function retefuenteByActivity(invoices = []) {
  ...
  invoices.forEach(inv => {
    ...
    if (!groups[key]) {
      ...
      groups[key] = { activity: key, label: meta.label, rate: meta.rate, base: 0, retencion: 0, count: 0 };
    }
    groups[key].base += Number(inv.base_amount) || 0;
    groups[key].retencion += Number(inv.retefuente_amount) || 0;
    groups[key].count += 1;
  });
  return Object.values(groups).filter(g => g.retencion > 0).sort((a, b) => b.retencion - a.retencion);
}
...
function tableTheme() {
  return {
    styles: { fontSize: 8.5, cellPadding: 4, textColor: COLOR_TEXT, lineColor: [226, 232, 240], lineWidth: 0.1 },
    headStyles: { fillColor: COLOR_BRAND, textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 8 },
...
export function generateForm300(summary, company, period, invoices = []) {
  ...
  const breakdown = ivaBreakdownByRate(invoices);
  const totalBaseGravable = breakdown.filter(b => b.rate > 0).reduce((s, b) => s + b.base, 0);
  const totalBaseExenta = breakdown.filter(b => b.rate === 0).reduce((s, b) => s + b.base, 0);

  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text('I. DESGLOSE DE IVA POR TARIFA', 14, y);
  y += 4;
  ...
  autoTable(doc, {
    ...
    body: breakdown.length ? breakdown.map(b => [
      `${b.rate}%`,
      String(b.count),
      formatCOP(b.base),
      formatCOP(b.iva),
    ...
    foot: [[
      ...
      { content: formatCOP(summary.total_base), styles: { fontStyle: 'bold' } },
      { content: formatCOP(summary.total_iva), styles: { fontStyle: 'bold' } },
    ]],
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text('II. RENGLONES DEL FORMULARIO 300', 14, y);
  y += 4;
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['32', 'Total base gravable', formatCOP(totalBaseGravable)],
      ['41', 'Total IVA generado', formatCOP(summary.total_iva)],
      ['60', 'Retenciones de IVA practicadas', formatCOP(summary.total_reteiva)],
      [
        { content: '67', styles: { fontStyle: 'bold' } },
        { content: 'Saldo a pagar / a favor', styles: { fontStyle: 'bold' } },
        { content: formatCOP(summary.total_iva - summary.total_reteiva), styles: { fontStyle: 'bold' } },
      ],
    ],
...
export function generateForm350(summary, company, period, invoices = []) {
  ...
  autoTable(doc, {
    ...
    body: refByActivity.length ? refByActivity.map(g => [
      g.label,
  ...
  autoTable(doc, {
    ...
    body: reteicaCities.length ? reteicaCities.map(g => [
